...
- 첫 화면 실제 운영 일정을 Google Calendar `일정_NEST` 기준으로 표시
...
6. Google Calendar 운영 일정 입력 규칙 정리
7. 운영 비밀번호와 관리자 권한 관리 기준 확정
8. Supabase 데이터 백업 절차 정리
...
## 2026-04-30 Google Calendar 일정 반영

- 첫 화면의 기존 `이번 달 주요 일정`을 Google Calendar 기반 `실제 운영 일정`으로 교체
- `일정_NEST` 캘린더를 서비스 계정으로 읽어 `오늘 일정`, `이번 주 일정`, `앞으로 30일` 3개 영역으로 표시
- `GOOGLE_CALENDAR_ID`가 없을 경우 서비스 계정 캘린더 목록에서 `일정_NEST` 이름으로 자동 탐색
- Google Calendar API 사용 설정과 캘린더 공유 권한이 필요
