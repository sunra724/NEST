...
담당자 입력 시트는 `https://docs.google.com/spreadsheets/d/1MBs3E6adF_wK5qlr092eB6l7jfLLLItNFMkwrHkzGWg/edit`을 기준으로 합니다. 서버 동기화는 기본적으로 `대시보드_입력` 탭을 읽습니다.
...
`id`는 대시보드 예산 상세 항목의 `N-0003` 같은 값과 일치해야 합니다. `품의상태`는 `품의 전`, `품의 요청`, `품의 승인`, `지출완료`, `보완필요`를 사용합니다.
